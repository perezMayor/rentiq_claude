// Quick test script — generates sample contract PDF
// Run from project root: node scripts/gen-contract-pdf.mjs
...
const store = JSON.parse(fs.readFileSync('.rentiq-v3-data/rental-store.json', 'utf8'));
...
const CW = PW - ML - MR;
const C1W = 249, CGAP = 17, C2W = CW - C1W - CGAP, C2X = ML + C1W + CGAP;
const PRIMARY = '#2b6cbd', BORDER_CLR = '#d1d5db', LABEL_CLR = '#6b7280';
const TXT_CLR = '#111827', SUBHDR_BG = '#f3f4f6', SUBHDR_CLR = '#9ca3af', TOTAL_BG = '#eff6ff';
...
function blankLine(n = 20) { return '_'.repeat(n); }
function fuelStr(fuel, lang) {
  if (!fuel) return 'N/D';
  if (lang.startsWith('en')) return ({GASOLINA:'Petrol',DIESEL:'Diesel',ELECTRICO:'Electric',HIBRIDO:'Hybrid'}[fuel] ?? fuel);
  return ({GASOLINA:'Gasolina',DIESEL:'Diésel',ELECTRICO:'Eléctrico',HIBRIDO:'Híbrido'}[fuel] ?? fuel);
}

// ── Draw helpers ──────────────────────────────────────────────────────────────
function hline(doc, y, x1=ML, x2=PW-MR, color=BORDER_CLR, lw=0.5) {
  doc.save().moveTo(x1,y).lineTo(x2,y).strokeColor(color).lineWidth(lw).stroke().restore();
}
function secBox(doc, title, x, y, w, h) {
  doc.save().roundedRect(x,y,w,h,2).strokeColor(BORDER_CLR).lineWidth(0.5).stroke().restore();
  doc.font('Helvetica-Bold').fontSize(8.5).fillColor(PRIMARY).text(title, x+7, y+6, {width:w-14, lineBreak:false});
  hline(doc, y+17, x+2, x+w-2, BORDER_CLR, 0.3);
  return y+20;
}
function field(doc, label, value, x, y, w) {
  doc.font('Helvetica').fontSize(6).fillColor(LABEL_CLR).text(label, x, y, {width:w, lineBreak:false});
  doc.font('Helvetica').fontSize(7.5).fillColor(TXT_CLR).text(value||'N/D', x, y+7, {width:w, lineBreak:false, ellipsis:true});
}
function subHdr(doc, text, x, y, w) {
  doc.save().rect(x,y,w,11).fillColor(SUBHDR_BG).fill().restore();
  doc.font('Helvetica-Bold').fontSize(6.5).fillColor(SUBHDR_CLR).text(text, x+4, y+3, {width:w-8, lineBreak:false, align:'center'});
  return y+13;
}

// ── Header ────────────────────────────────────────────────────────────────────
function drawHeader(doc, s, contractNum, copyLabel, lang) {
  const tr = strings(lang);
  let y = 22;
  if (s.logoDataUrl) {
    try {
      const b64 = s.logoDataUrl.replace(/^data:image\/\w+;base64,/, '');
      doc.image(Buffer.from(b64,'base64'), ML, y, {height:44, fit:[100,44]});
    } catch {}
  }
  const cx = ML+108, cw = CW-108-120;
  const name = s.documentName ?? s.name ?? '';
  const nif  = s.taxId ?? s.nif ?? '';
  const addr = s.fiscalAddress ?? s.address ?? '';
  doc.font('Helvetica-Bold').fontSize(11).fillColor(TXT_CLR).text(name, cx, y+2, {width:cw, lineBreak:false});
  doc.font('Helvetica').fontSize(7.5).fillColor(LABEL_CLR).text(nif, cx, y+15, {width:cw, lineBreak:false});
  doc.font('Helvetica').fontSize(7.5).fillColor(LABEL_CLR).text(addr, cx, y+24, {width:cw, lineBreak:false});
  doc.font('Helvetica-Bold').fontSize(11).fillColor(TXT_CLR).text(`${tr.contractNo} ${contractNum}`, PW-MR-118, y+2, {width:118, align:'right', lineBreak:false});
  const badgeY = y+18;
  doc.save().roundedRect(PW-MR-102, badgeY, 102, 15, 3).fillColor(PRIMARY).fill().restore();
  doc.font('Helvetica-Bold').fontSize(7.5).fillColor('white').text(copyLabel, PW-MR-102, badgeY+4, {width:102, align:'center', lineBreak:false});
  y = 74;
  hline(doc, y, ML, PW-MR, PRIMARY, 1.5);
  y += 8;
  doc.font('Helvetica-Bold').fontSize(18).fillColor(TXT_CLR).text(tr.titleDoc, ML, y, {lineBreak:false});
  y += 26;
...
// ── Anverso ───────────────────────────────────────────────────────────────────
function renderAnverso(doc, data, copyLabel, isBlank) {
  const { contract, client, vehicle, model, category, settings, language: lang } = data;
  const tr = strings(lang);
  const v = (val, fb='N/D') => isBlank ? blankLine() : (val != null && String(val).trim() !== '' ? String(val) : fb);
  ...
  for (let i = 0; i < vFields.length; i++) {
    ...
    if (i < vFields.length-1) {
      doc.save().moveTo(vx-4, vInner+2).lineTo(vx-4, y+VH-4).strokeColor(BORDER_CLR).lineWidth(0.3).stroke().restore();
    }
  }
  y += VH+4;

  // Row 1: Conductor | Alquiler
  let ly = y, ry = y;
  const cpH = 154;
  ...
  doc.font('Helvetica').fontSize(7.5).fillColor(TXT_CLR).text(v(addrStr !== 'N/D' ? addrStr : undefined), ML+7, cpY, {width:C1W-14, lineBreak:false, ellipsis:true});
  cpY += 10;
  ...
  daY += 20;
  field(doc, tr.deliveryFlight, 'N/D', C2X+7, daY, rc2w);
  field(doc, tr.returnFlight, 'N/D', C2X+7+rc2w+4, daY, rc2w);
  daY += 20;
  ...
  doc.font('Helvetica').fontSize(7.5).fillColor(LABEL_CLR).text(tr.noAddDrivers, ML+7, ly+24, {lineBreak:false});
  ly += caH+4;
  const dtH = 44;
  ...
  function dtRow(lbl, val) {
    const lw = C2W*0.7;
    doc.font('Helvetica').fontSize(6.5).fillColor(LABEL_CLR).text(lbl, C2X+7, dtY, {width:lw, lineBreak:false});
    doc.font('Helvetica').fontSize(7).fillColor(TXT_CLR).text(val, C2X+lw, dtY, {width:C2W-lw-8, align:'right', lineBreak:false});
    dtY += 9;
  }
  ...
  // Row 3: Empresa | Desglose
  ly=y; ry=y;
  const emH = 72;
  const emInner = secBox(doc, tr.companySection, ML, ly, C1W, emH);
  let emY = emInner+4;
  const emPairs = [[tr.companySection, blankLine(22)],[tr.companyCIF,blankLine(14)],[tr.fiscalAddr,blankLine(22)],[tr.contact,blankLine(22)]];
  ...
  for (const [lbl, amt, bold] of rows) {
    ...
    if (bold) doc.save().rect(C2X+2, deY-2, C2W-4, 14).fillColor(TOTAL_BG).fill().restore();
    const lw = C2W*0.62, vw = C2W-lw-10;
    doc.font(bold?'Helvetica-Bold':'Helvetica').fontSize(7.5).fillColor(bold?PRIMARY:LABEL_CLR).text(lbl, C2X+7, deY, {width:lw, lineBreak:false});
    doc.font(bold?'Helvetica-Bold':'Helvetica').fontSize(7.5).fillColor(TXT_CLR).text(valStr, C2X+lw, deY, {width:vw, align:'right', lineBreak:false});
    deY += rowH;
  }
  deY += 2;
  ...
  deY += 4;
  const lw2 = C2W*0.62;
  doc.font('Helvetica').fontSize(7.5).fillColor(LABEL_CLR).text(tr.franchise, C2X+7, deY, {width:lw2, lineBreak:false});
  doc.font('Helvetica').fontSize(7.5).fillColor(TXT_CLR).text('N/D', C2X+lw2, deY, {width:C2W-lw2-10, align:'right', lineBreak:false});
  ry += deH+4;
  y = Math.max(ly, ry);

  // Row 4: Vehículo | Cambios + Obs
  ly=y; ry=y;
  const vehH = 90;
  secBox(doc, tr.vehicleSection, ML, ly, C1W, vehH);
  const vbx=ML+8, vby=ly+22, vbw=C1W-16, vbh=vehH-28;
  doc.save().rect(vbx,vby,vbw,vbh).fillColor('#f9fafb').fill().strokeColor('#e5e7eb').lineWidth(0.4).stroke().restore();
  if (!isBlank && contract?.plate) {
    doc.font('Helvetica-Bold').fontSize(8).fillColor('#d1d5db').text(contract.plate, vbx, vby+vbh/2-5, {width:vbw, align:'center', lineBreak:false});
  }
  ly += vehH+4;
  const chH = 38;
  secBox(doc, tr.vehicleChanges, C2X, ry, C2W, chH);
  doc.font('Helvetica').fontSize(7.5).fillColor(LABEL_CLR).text(tr.noChanges, C2X+7, ry+23, {lineBreak:false});
  ry += chH+4;
  const obsText = isBlank ? '' : (contract?.notes ?? '');
  const obsH = vehH-chH-4;
  secBox(doc, tr.observations, C2X, ry, C2W, obsH);
  if (obsText) {
    doc.font('Helvetica').fontSize(7.5).fillColor(TXT_CLR).text(obsText, C2X+7, ry+22, {width:C2W-14, height:obsH-28, lineBreak:true});
  } else {
    doc.font('Helvetica').fontSize(7.5).fillColor(LABEL_CLR).text(tr.noObs, C2X+7, ry+23, {lineBreak:false});
  }
  ry += obsH+4;
  y = Math.max(ly, ry)+4;
  ...
  hline(doc, y, ML, PW-MR, BORDER_CLR, 0.5);
  y += 6;
  doc.font('Helvetica').fontSize(6.5).fillColor(LABEL_CLR).text(tr.sigTextEs, ML, y, {width:CW, lineBreak:false});
  y += 9;
  doc.font('Helvetica').fontSize(6.5).fillColor(LABEL_CLR).text('* '+tr.sigTextEn, ML, y, {width:CW, lineBreak:false});
  y += 14;
  const sigW = CW/2-8, sigH = 44;
  doc.save().roundedRect(ML, y, sigW, sigH, 2).strokeColor(BORDER_CLR).lineWidth(0.5).stroke().restore();
  doc.font('Helvetica-Bold').fontSize(8).fillColor(TXT_CLR).text(tr.sigRenter, ML+6, y+5, {lineBreak:false});
  const sx2 = ML+sigW+16;
  doc.save().roundedRect(sx2, y, sigW, sigH, 2).strokeColor(BORDER_CLR).lineWidth(0.5).stroke().restore();
  doc.font('Helvetica-Bold').fontSize(8).fillColor(TXT_CLR).text(tr.sigCompany, sx2+6, y+5, {lineBreak:false});
  y += sigH+6;

  // Footer
  const footerText = settings.documentFooter ?? [settings.documentName??settings.name??'', settings.fiscalAddress??settings.address??''].filter(Boolean).join('  ·  ');
  hline(doc, PH-22, ML, PW-MR, BORDER_CLR, 0.3);
  doc.font('Helvetica').fontSize(6.5).fillColor(LABEL_CLR).text(footerText, ML, PH-16, {width:CW, align:'center', lineBreak:false});
}

// ── Reverso (two-column bilingüe) ─────────────────────────────────────────────
function renderReverso(doc, settings) {
  const MT=22, MB=22;
  const colW = Math.floor((CW-14)/2);
  const col2x = ML+colW+14;
  const titleY = MT;

  doc.font('Helvetica-Bold').fontSize(8.5).fillColor(PRIMARY).text('CONDICIONES GENERALES DEL CONTRATO', ML, titleY, {width:colW, lineBreak:false});
  doc.font('Helvetica-Bold').fontSize(8.5).fillColor(PRIMARY).text('CAR RENTAL CONTRACT', col2x, titleY, {width:colW, lineBreak:false});
  ...
  doc.save().moveTo(divX, contentY-4).lineTo(divX, contentY+contentH).strokeColor(BORDER_CLR).lineWidth(0.4).stroke().restore();

  if (esContent) {
    doc.font('Helvetica').fontSize(fontSize).fillColor(TXT_CLR).text(esContent, ML, contentY, {width:colW, height:contentH, align:'justify', lineBreak:true});
  } else {
    doc.font('Helvetica').fontSize(8).fillColor(LABEL_CLR).text('Configura el contenido en Gestor → Empresa → Contenido reverso contrato.', ML, contentY, {width:colW});
  }
  if (enContent) {
    doc.font('Helvetica').fontSize(fontSize).fillColor(TXT_CLR).text(enContent, col2x, contentY, {width:colW, height:contentH, align:'justify', lineBreak:true});
  } else {
    doc.font('Helvetica').fontSize(8).fillColor(LABEL_CLR).text('Add English content in Settings → Company → Contract back content (EN).', col2x, contentY, {width:colW});
  }
}

// ── Build ─────────────────────────────────────────────────────────────────────
const data = { contract, client, vehicle, model, category, branch, settings, language };
const tr = strings(language);

const doc = new PDFDocument({ size:'A4', margin:0, autoFirstPage:true });
const outPath = '/tmp/contrato-test.pdf';
...
out.on('finish', () => {
  ...
  console.log('Tamaño:', fs.statSync(outPath).size, 'bytes');
});
