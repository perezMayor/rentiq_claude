...
function vline(doc, x, y1, y2, color = BORDER_CLR, lw = 0.3) {
...
function drawHeader(doc, s, contractNum, copyLabel, lang) {
  ...
  const phone = s.companyPhone ?? s.phone ?? '';
...
function renderAnverso(doc, data, copyLabel, isBlank) {
  ...
  const VH  = 50;
  const VIW = CW - 14;   // inner width (7px padding each side)
  ...
  doc.font('Helvetica').fontSize(6.5).fillColor(LABEL_CLR)
     .text(trEs.sigTextEs, ML, y, { width: CW, lineBreak: false });
  y += 9;
  doc.font('Helvetica').fontSize(6.5).fillColor(LABEL_CLR)
  ...
  doc.font('Helvetica-Bold').fontSize(8).fillColor(TXT_CLR).text(tr.sigCompany, sx2 + 7, y + 5, { lineBreak: false });
...
function renderReverso(doc, settings) {
  ...
  if (esContent) {
    ...
    doc.font('Helvetica').fontSize(8).fillColor(LABEL_CLR)
       .text('Configura el contenido en Gestor → Empresa → Contenido reverso contrato.', ML, contentY, { width: colW });
  }
  if (enContent) {
    doc.font('Helvetica').fontSize(fontSize).fillColor(TXT_CLR)
